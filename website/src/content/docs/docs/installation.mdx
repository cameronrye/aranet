---
title: Installation
description: How to install the Aranet CLI on macOS, Linux, and Windows.
---

import { Steps, Tabs, TabItem, Aside, Badge, Card, CardGrid, Code } from '@astrojs/starlight/components';
import { CURRENT_RELEASE, CURRENT_RELEASE_TAG } from '../../../site-config.mjs';

<Badge text={CURRENT_RELEASE_TAG} variant="success" /> Latest Release

## Pre-built Binaries

The fastest way to install Aranet CLI is using pre-built binaries:

<Tabs>
  <TabItem label="Homebrew (macOS, Linux x86_64)">
```bash
brew tap cameronrye/aranet
brew install aranet
```
  </TabItem>
  <TabItem label="Shell (macOS/Linux)">
```bash
curl --proto '=https' --tlsv1.2 -LsSf \
  https://github.com/cameronrye/aranet/releases/latest/download/aranet-cli-installer.sh | sh
```
  </TabItem>
  <TabItem label="PowerShell (Windows)">
```powershell
powershell -ExecutionPolicy ByPass -c "irm https://github.com/cameronrye/aranet/releases/latest/download/aranet-cli-installer.ps1 | iex"
```
  </TabItem>
</Tabs>

On a minimal Linux system, such as a container image, first install `curl`, `ca-certificates`, `xz-utils` and the D-Bus library (for example `apt-get update && apt-get install -y curl ca-certificates xz-utils libdbus-1-3`, as root or with `sudo`): the installer downloads with `curl` and unpacks a `.tar.xz` archive, and `aranet` needs `libdbus-1.so.3` to talk to BlueZ.

The prebuilt Linux binaries need glibc 2.35 or newer: Ubuntu 22.04, Debian 12, Raspberry Pi OS Bookworm, Fedora 36 or later. On an older system, build from source instead (see [Downloads](/aranet/docs/downloads/#source-code)).

## Download Direct

Download binaries directly from GitHub Releases:

<CardGrid>
  <Card title="macOS (Apple Silicon)" icon="apple">
    [aranet-cli-aarch64-apple-darwin.tar.xz](https://github.com/cameronrye/aranet/releases/latest/download/aranet-cli-aarch64-apple-darwin.tar.xz)
  </Card>
  <Card title="macOS (Intel)" icon="apple">
    [aranet-cli-x86_64-apple-darwin.tar.xz](https://github.com/cameronrye/aranet/releases/latest/download/aranet-cli-x86_64-apple-darwin.tar.xz)
  </Card>
  <Card title="Linux (x86_64)" icon="linux">
    [aranet-cli-x86_64-unknown-linux-gnu.tar.xz](https://github.com/cameronrye/aranet/releases/latest/download/aranet-cli-x86_64-unknown-linux-gnu.tar.xz)
  </Card>
  <Card title="Linux (ARM64)" icon="linux">
    [aranet-cli-aarch64-unknown-linux-gnu.tar.xz](https://github.com/cameronrye/aranet/releases/latest/download/aranet-cli-aarch64-unknown-linux-gnu.tar.xz)
  </Card>
  <Card title="Windows (x86_64)" icon="seti:windows">
    [aranet-cli-x86_64-pc-windows-msvc.zip](https://github.com/cameronrye/aranet/releases/latest/download/aranet-cli-x86_64-pc-windows-msvc.zip)
  </Card>
</CardGrid>

<Aside>
  All releases include SHA256 checksums. View all versions on the [GitHub Releases](https://github.com/cameronrye/aranet/releases) page.
</Aside>

## Docker

Run aranet-service in a container (requires BLE access on the host):

```bash
docker compose up -d
# Dashboard: http://localhost:8080/dashboard
```

Or use the full monitoring stack with Prometheus and Grafana:

```bash
docker compose -f docker/docker-compose.yml up -d
```

See [Service Deployment](/aranet/docs/service/deployment/#docker) for configuration details.

## Install from crates.io

Install using Cargo (requires Rust toolchain):

```bash
cargo install aranet-cli
```

This installs the `aranet` executable.

## Build from Source

Alternatively, build from source for the latest development version:

<Steps>
1. **Clone the repository**

   ```bash
   git clone https://github.com/cameronrye/aranet.git
   cd aranet
   ```

2. **Build all crates**

   ```bash
   cargo build --release
   ```

3. **Run the CLI**

   ```bash
   ./target/release/aranet --help
   ```
</Steps>

## Platform-Specific Notes

<Tabs>
  <TabItem label="macOS">
    No additional dependencies required. Bluetooth access will prompt for permission on first use.
  </TabItem>
  <TabItem label="Linux">
    Install the D-Bus development library:
    ```bash
    # Debian/Ubuntu
    sudo apt-get install libdbus-1-dev

    # Fedora
    sudo dnf install dbus-devel

    # Arch
    sudo pacman -S dbus
    ```
  </TabItem>
  <TabItem label="Windows">
    No additional dependencies required. Ensure Bluetooth is enabled in Windows settings.
  </TabItem>
</Tabs>

## Installing the CLI Globally

To install the CLI to your Cargo bin directory:

```bash
cargo install --path crates/aranet-cli
```

This allows you to run `aranet` from anywhere:

```bash
aranet scan
```

## Shell Completions

Generate shell completions for your shell:

<Tabs>
  <TabItem label="Bash">
    ```bash
    aranet completions bash > ~/.local/share/bash-completion/completions/aranet
    ```
  </TabItem>
  <TabItem label="Zsh">
    ```bash
    aranet completions zsh > ~/.zfunc/_aranet
    ```
  </TabItem>
  <TabItem label="Fish">
    ```bash
    aranet completions fish > ~/.config/fish/completions/aranet.fish
    ```
  </TabItem>
</Tabs>

## Using as a Library

Add `aranet-core` to your `Cargo.toml`:

<Code lang="toml" code={`[dependencies]\naranet-core = "${CURRENT_RELEASE.series}"`} />

## Verify Installation

If you used the shell or PowerShell installer, open a new terminal first. Then check that everything is working:

```bash
# Show version
aranet --version

# Run BLE diagnostics
aranet doctor

# Scan for devices (requires Bluetooth)
aranet scan --timeout 5
```
