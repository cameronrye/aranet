...
<Aside type="note">
  WebSocket connections are subject to the same authentication requirements as REST endpoints. See [API Security](/aranet/docs/service/security/) for configuration details.
</Aside>
...
URL-encode the key as shown. Keys sent unencoded are accepted too, with `+`, `/` and `=` read literally.
