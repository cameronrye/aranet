...
![CLI Demo](/aranet/screenshots/cli-read.gif)
...
## Sync to the Local Cache

Pull history into the local SQLite cache so you can query it later without reconnecting:

```bash
# Sync a specific device
aranet sync --device AA:BB:CC:DD:EE:FF

# Or sync every remembered device
aranet sync --all
```

Generate a summary report from cached data:

```bash
# Daily report for one device
aranet report --device living-room

# Weekly report for all cached devices
aranet report --all --period weekly --format json
```

## Start the API Server and Dashboard

The CLI can launch the bundled `aranet-service` HTTP API and dashboard:
...
- Deploy the [background service](/aranet/docs/service/deployment/)
