...
| 4 | GUI Application | <Badge text="DONE" variant="success" /> | Full MVP with charts and multi-device |
| 5 | Data Persistence & API | <Badge text="DONE" variant="success" /> | aranet-store and aranet-service complete |
| 6 | Integrations | <Badge text="DONE" variant="success" /> | Prometheus, MQTT, Webhooks, InfluxDB, mDNS, Grafana |
...
## Phase 4: GUI Application (Complete)
...
<CardGrid>
  <Card title="Prometheus Metrics" icon="bars">
    `/metrics` endpoint with full sensor data for Grafana dashboards
  </Card>
  <Card title="MQTT Publisher" icon="external">
    Broadcast readings to MQTT brokers for Home Assistant and IoT
  </Card>
  <Card title="Webhooks & InfluxDB" icon="warning">
    HTTP POST alerts for threshold breaches and InfluxDB v2 export
  </Card>
  <Card title="mDNS & Grafana" icon="external">
    Automatic service discovery and pre-built Grafana dashboard
  </Card>
</CardGrid>
...
- **Webhooks**: HTTP POST alerts for CO₂, radon, and battery thresholds (Slack/Discord/PagerDuty)
- **InfluxDB**: Real-time export to InfluxDB v2 via line protocol
- **mDNS**: Automatic network discovery via `_aranet._tcp.local.`
- **Grafana**: Pre-built dashboard template with gauges, time series, and collector stats
...
- [x] GUI app installable on all platforms
- [x] Published to crates.io as `aranet-cli`
...
4. **Integrations** — Home Assistant native component, additional IoT platforms
